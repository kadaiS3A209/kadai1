...
.page-header {width: 100%; max-width: 1400px; margin-bottom: 10px; text-align: left;}
.button {padding: 8px 15px; color: white; background-color: #007bff;border: none;border-radius: 4px; cursor: pointer; text-decoration: none; font-size: 14px; display: inline-block;}
.button-back {background-color: #6c757d; /* 戻るボタンの色をグレー系に */}
.button-back:hover {background-color: #5a6268;}
</style>
</head>
<body>
　<div class="page-header">
       <a href="ReturnToMenuServlet" class="button button-back">メニューに戻る</a>
   </div>
  ...
  ws.onerror = (error) => { $status.text('エラーが発生しました').css('background-color', 'red'); console.error("WebSocket Error:", error); };
   ws.onmessage = (event) => {
      try {
          ...
          switch (msg.type) {
          case 'text': { // ブロックスコープを作成
              // 常にテキストエリアの内容は更新する
              // ただし、カーソル位置を保持することで、自分自身の入力が阻害されるのを防ぐ
              const memoPadElement = $memoPad[0];
              const currentPosition = memoPadElement.selectionStart;
              const originalText = memoPadElement.value;
              // サーバーからのテキストで更新
              memoPadElement.value = msg.data;
              // もし、この更新が自分以外のユーザーによるもので、
              // かつ自分自身がテキストエリアを編集中だった場合、カーソル位置を再計算して設定する
              if (document.activeElement === memoPadElement) {
                  // 文字列の長さを比較して、カーソル位置を調整
                  const newPosition = currentPosition + (memoPadElement.value.length - originalText.length);
                  memoPadElement.setSelectionRange(newPosition, newPosition);
              }
              break;
          }
              case 'draw':
